...
swjsq_login() {
	...
	json_close_object
		_log "模拟机型：$fastd1ck_brand $fastd1ck_model"  
		_log "设备标识：$fastd1ck_devid"
...
isp_upgrade() {
	...
	case ${lasterr:=-1} in
		0)
			...
			local outmsg="${link_cn}带宽提速成功!"; _log "$outmsg" $(( 1 | $1 * 8 ))
			[ $1 -eq 1 ] && down_acc=2 || up_acc=2
			;;
		812) # 812 已处于提速状态
			lasterr=0
			local outmsg="${link_cn}当前宽带已处于提速状态"; _log "$outmsg" $(( 1 | $1 * 8 ))
			[ $1 -eq 1 ] && down_acc=2 || up_acc=2
			;;
		724) # 724 账号存在异常
			lasterr=-2
